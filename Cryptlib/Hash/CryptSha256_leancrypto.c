/* SHA-256 Digest Wrapper Implementation using leancrypto.
 *
...
{
	/*
	 * Retrieves leancrypto SHA-256 Context Size
	 *
	 * Add the memory for proper alignment in the calls below
	 */
	return (UINTN)(LC_SHA256_CTX_SIZE + LC_HASH_COMMON_ALIGNMENT);
}
...
BOOLEAN EFIAPI Sha256Init (OUT VOID *Sha256Context)
{
	...
	ctx = (struct lc_hash_ctx *)LC_ALIGN_PTR_64(Sha256Context,
						    LC_HASH_COMMON_ALIGNMENT);

	/* leancrypto SHA-256 Context Initialization */
	LC_SHA256_CTX(ctx);
	lc_hash_init(ctx);
				...
				OUT VOID *NewSha256Context)
{
	...
	CopyMem(NewSha256Context, (void *)Sha256Context,
		LC_SHA256_CTX_SIZE + LC_HASH_COMMON_ALIGNMENT);
 ...
 * initialized by Sha256Init(), and should not be finalized
 ...
 */
BOOLEAN EFIAPI Sha256Update (IN OUT VOID *Sha256Context,IN CONST VOID *Data,
			     IN UINTN DataSize)
{
	struct lc_hash_ctx *ctx;
	...
	/* leancrypto SHA-256 Hash Update */
 ...
 * SHA-256 context cannot be used again. SHA-256 context should be already
...
BOOLEAN EFIAPI Sha256Final (IN OUT VOID *Sha256Context, OUT UINT8 *HashValue)
{
	...
	/* leancrypto SHA-256 Hash Finalization */
 ...
 *
 * This function performs the SHA-256 message digest of a given data buffer, and
 ...
 * @retval FALSE SHA-256 digest computation failed.
 */
BOOLEAN EFIAPI Sha256HashAll (IN CONST VOID *Data, IN UINTN DataSize,
			      OUT UINT8 *HashValue)
{
	/* Check input parameters. */
	if (HashValue == NULL) {
	...
	/* leancrypto SHA-256 Hash Computation. */
	lc_hash(lc_sha256, Data, DataSize, HashValue);
